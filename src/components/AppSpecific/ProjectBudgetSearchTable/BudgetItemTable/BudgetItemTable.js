...
const ActionCell = ({ rowData, dataKey, handleChangeDelete, handleChangeEdit, ...props }) => {
...
const CheckCell = ({ rowData, handleChangeCheck, checkedKeys, dataKey, ...props }) => (
    ...
    const filterBuilderData = (data, query) => {
        ...
        data.forEach((item) => {
            for (let key in query) {
                if (!_.includes(searchedData, item) && item[key].toString().toLowerCase().includes(query[key][0].toString().toLowerCase())) {
        ...
        <React.Fragment>
            ...
            >
                <Column width={50} align="center">
                    <HeaderCell style={{ color: colors.primary, fontWeight: 900, fontSize: 'larger' }}>
                        {"Select"}
                    </HeaderCell>
